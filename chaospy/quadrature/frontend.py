...
def generate_quadrature(
    ...
    if sparse:
        return chaospy.quadrature.sparse_grid(
            order=order,
            dist=dist,
            growth=growth,
...
def _generate_quadrature(order, dist, rule, **kwargs):
    ...
    if isinstance(dist, (chaospy.Add, chaospy.Multiply)):
        ...
        opr = add if isinstance(dist, chaospy.Add) else mul
        abscissas = opr(abscissas.T, const.T).T
        return abscissas, weights
