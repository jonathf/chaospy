...
class MeanCovarianceDistribution(Distribution):
    ...
    def get_parameters(self, idx, cache, assert_numerical=True):
        ...
        if isinstance(mean, Distribution):
            mean = [mean._get_cache(dim, cache, get=0) for dim in range(len(mean))]
            if any([isinstance(condition, chaospy.Distribution) for condition in mean]):
        ...
        return dict(
            idx=idx, mean=mean, sigma=sigma, dim=dim, mut=mu_transform, cache=cache
        )
