...
class OperatorDistribution(Distribution):
    ...
    def get_parameters(self, idx, cache, assert_numerical=True):
        ...
        if isinstance(parameters["left"], Distribution):
            parameters["left"] = parameters["left"]._get_cache(
        ...
        if isinstance(parameters["right"], Distribution):
            parameters["right"] = parameters["right"]._get_cache(
                idx, cache=parameters["cache"], get=0
            )
        ...
        if assert_numerical:
            assert not isinstance(parameters["left"], Distribution) or not isinstance(
                parameters["right"], Distribution
            )
