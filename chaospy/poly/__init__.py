...
it creates an array of simple monomials::
    ...
    >>> print(chaospy.basis(start=4))
    [1 q0 q0**2 q0**3 q0**4]

It can be expanded to include number of dimensions and a lower bound for the
polynomial order::

    >>> print(chaospy.basis(start=1, stop=2, dim=2))
    [q1 q0 q1**2 q0*q1 q0**2]

There is also the possibility to create anisotropic expansions::

    >>> print(chaospy.basis(start=1, stop=[1, 2]))
    [q1 q0 q1**2 q0*q1 q0*q1**2]

and the possibility to fine tune the sorting of the polynomials::

    >>> print(chaospy.basis(start=1, stop=2, dim=2, sort="GRI"))
    [q1**2 q0*q1 q0**2 q1 q0]
...
from numpoly import (
    abs, absolute, add, all, any, around, atleast_1d, atleast_2d, atleast_3d,
    ceil, concatenate, decompose, diff, divide, dstack, equal, floor, gradient,
    hessian, hstack, inner, isclose, isconstant, isfinite, multiply, ndpoly,
    negative, not_equal, outer, positive, power, prod, repeat, rint, round,
    square, stack, subtract, sum, vstack,
)
