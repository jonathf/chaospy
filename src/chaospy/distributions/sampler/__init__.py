...
:func:`~chaospy.distributions.samplers.generator.generate_samples`. These
samples are then mapped using a :ref:`rosenblatt` to map the values on the
domain respective to the distribution in question. This way, all variance
...
sequences, it is also possible to take a look at
:mod:`chaospy.distributions.sampler.generator.sequences`. This module will
contain all the individual sequence schemes with extra flags for controlling
each scheme. In the case of the Krobov lattice::
